...
def list_webhooks(_admin=Depends(require_role("admin"))):
    return [WebhookOut(**w) for w in webhook_service.list_webhooks()]


@router.post("", response_model=WebhookOut, status_code=201)
def create_webhook(req: WebhookCreateRequest, _admin=Depends(require_role("admin"))):
    try:
        wh = webhook_service.create_webhook(req.url, req.secret, req.event_types)
...
def update_webhook(webhook_id: str, req: WebhookUpdateRequest, _admin=Depends(require_role("admin"))):
    try:
...
def delete_webhook(webhook_id: str, _admin=Depends(require_role("admin"))):
