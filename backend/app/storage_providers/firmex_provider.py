"""Firmex (virtual data rooms), via a REST API assumed to follow Firmex's
general documented conventions (JSON, token auth against a per-connection
Firmex site URL).

UNVERIFIED and LOW CONFIDENCE — Firmex's public API documentation is
sparse compared to most other providers in this codebase (it's a
data-room product whose primary interface is its own web UI, with API
access typically granted per-customer rather than broadly published).
Endpoint paths, field names, and the overall shape below are a best-
effort reconstruction from Firmex's general "data room > folder >
document" model and common patterns for this category of product (VDRs),
NOT a verified spec. Treat this adapter as a structural starting point
that needs real endpoint verification against Firmex's actual API
documentation (available to Firmex customers) before any production use
— the least-confident adapter in this codebase alongside ShareVault,
Intralinks, HighQ, and Workshare, all built the same way for the same
reason: no public API reference was available to write against.

Firmex data rooms are the top-level container (analogous to Box's
"account root"); this connection picks the account's first data room as
its scope. Trash is emulated via a hidden folder since there is no
confidently-known native trash API; version history is emulated as a
single current version for the same reason.
"""
...
class FirmexProvider(StorageProvider):
    key = "firmex"
    display_name = "Firmex"
    ...
    def config_fields(self) -> list[ConfigField]:
        return [ConfigField("base_url", "Firmex site URL", "https://yourcompany.firmex.com")]

    def _base_url(self, creds: dict) -> str:
        return creds["base_url"].rstrip("/")

    def _login(self, creds: dict) -> str:
        resp = requests.post(f"{self._base_url(creds)}/api/v1/auth/login",
                              json={"username": creds["username"], "password": creds["password"]}, timeout=30)
        if resp.status_code >= 400:
            raise ProviderError("Invalid Firmex credentials", status_code=401)
    ...
    def _request(self, creds: dict, method: str, path: str, **kwargs) -> requests.Response:
        ...
        try:
            ...
            raise ProviderError(f"Couldn't reach Firmex: {exc}", status_code=502)
        ...
        if resp.status_code >= 400:
            raise ProviderError(f"Firmex error {resp.status_code}: {resp.text[:300]}", status_code=502)
        return resp
    ...
    def authenticate(self, username: str, password: str, config: dict | None = None) -> dict | None:
        ...
        if not base_url:
            raise ProviderError("Firmex site URL is required", status_code=400)
    ...
    def _dataroom_id(self, creds: dict) -> str:
        rooms = self._request(creds, "GET", "/api/v1/datarooms").json()
        entries = rooms if isinstance(rooms, list) else rooms.get("data", [])
        if not entries:
            raise ProviderError("No Firmex data room is available on this account", status_code=502)
    ...
    def _find_or_create_child(self, creds: dict, room_id: str, parent_id: str | None, name: str) -> str:
        result = self._request(creds, "GET", f"/api/v1/datarooms/{room_id}/folders",
                                params={"parentId": parent_id or ""}).json()
        entries = result if isinstance(result, list) else result.get("data", [])
        for e in entries:
            if e.get("name") == name:
                return str(e.get("id"))
        created = self._request(creds, "POST", f"/api/v1/datarooms/{room_id}/folders",
    ...
    def _root_id(self, creds: dict) -> str:
        ...
        with self._root_id_lock:
            ...
            if cached:
                return cached
            room_id = self._dataroom_id(creds)
            creds["_room_id"] = room_id
            root_id = self._find_or_create_child(creds, room_id, None, _APP_ROOT_NAME)
    ...
    def _trash_id(self, creds: dict) -> str:
        ...
        with self._trash_id_lock:
            ...
            trash_id = self._find_or_create_child(creds, creds["_room_id"], root, _TRASH_NAME)
    ...
    def _room(self, creds: dict) -> str:
        self._root_id(creds)
        return creds["_room_id"]

    def get_children(self, creds: dict, folder_id: str | None) -> FolderContents:
        room = self._room(creds)
        node = self._resolve(creds, folder_id)
        fres = self._request(creds, "GET", f"/api/v1/datarooms/{room}/folders", params={"parentId": node}).json()
        ffiles = self._request(creds, "GET", f"/api/v1/datarooms/{room}/documents", params={"folderId": node}).json()
    ...
    def list_trash(self, creds: dict) -> FolderContents:
        ...
        fres = self._request(creds, "GET", f"/api/v1/datarooms/{room}/folders", params={"parentId": trash_id}).json()
        ffiles = self._request(creds, "GET", f"/api/v1/datarooms/{room}/documents", params={"folderId": trash_id}).json()
        f_entries = fres if isinstance(fres, list) else fres.get("data", [])
        file_entries = ffiles if isinstance(ffiles, list) else ffiles.get("data", [])
    ...
    def create_folder(self, creds: dict, parent_id: str | None, name: str) -> FolderInfo:
        ...
        created = self._request(creds, "POST", f"/api/v1/datarooms/{room}/folders",
    ...
    def rename_folder(self, creds: dict, folder_id: str, name: str) -> FolderInfo:
        room = self._room(creds)
        updated = self._request(creds, "PUT", f"/api/v1/datarooms/{room}/folders/{folder_id}",
                                 json={"name": name}).json()
    ...
    def delete_folder(self, creds: dict, folder_id: str) -> None:
        room = self._room(creds)
        self._request(creds, "DELETE", f"/api/v1/datarooms/{room}/folders/{folder_id}")
    ...
    def create_document(self, creds: dict, folder_id: str | None, name: str, content_type: str, content: bytes) -> FileInfo:
        ...
        created = self._request(creds, "POST", f"/api/v1/datarooms/{room}/documents",
    ...
    def get_file(self, creds: dict, file_id: str) -> FileInfo:
        room = self._room(creds)
        result = self._request(creds, "GET", f"/api/v1/datarooms/{room}/documents/{file_id}").json()
        return self._entry_to_file(result.get("data", result), None)

    def rename_file(self, creds: dict, file_id: str, name: str) -> FileInfo:
        room = self._room(creds)
        updated = self._request(creds, "PUT", f"/api/v1/datarooms/{room}/documents/{file_id}",
                                 json={"name": name}).json()
    ...
    def delete_file(self, creds: dict, file_id: str) -> None:
        room = self._room(creds)
        self._request(creds, "DELETE", f"/api/v1/datarooms/{room}/documents/{file_id}")

    def get_content(self, creds: dict, file_id: str) -> bytes:
        room = self._room(creds)
        return self._request(creds, "GET", f"/api/v1/datarooms/{room}/documents/{file_id}/content").content
    ...
    def create_version(self, creds: dict, file_id: str, content_type: str, content: bytes) -> FileInfo:
        room = self._room(creds)
        info = self.get_file(creds, file_id)
        self._request(creds, "POST", f"/api/v1/datarooms/{room}/documents/{file_id}/content",
    ...
    def trash_folder(self, creds: dict, folder_id: str) -> None:
        ...
        self._request(creds, "PUT", f"/api/v1/datarooms/{room}/folders/{folder_id}", json={"parentId": target})
    ...
    def trash_file(self, creds: dict, file_id: str) -> None:
        room = self._room(creds)
        target = self._trash_id(creds)
        self._request(creds, "PUT", f"/api/v1/datarooms/{room}/documents/{file_id}", json={"folderId": target})
    ...
    def search(self, creds: dict, query: str) -> tuple[list[FolderInfo], list[FileInfo]]:
        ...
        def walk(node_id, depth):
            if depth > 6 or node_id == trash:
                return
            fres = self._request(creds, "GET", f"/api/v1/datarooms/{room}/folders", params={"parentId": node_id}).json()
            f_entries = fres if isinstance(fres, list) else fres.get("data", [])
            for e in f_entries:
            ...
            ffiles = self._request(creds, "GET", f"/api/v1/datarooms/{room}/documents", params={"folderId": node_id}).json()
            file_entries = ffiles if isinstance(ffiles, list) else ffiles.get("data", [])
