"""Thomson Reuters HighQ, via a REST API assumed to follow HighQ's general
documented conventions (JSON, token auth against a per-connection HighQ
site URL).

UNVERIFIED and LOW CONFIDENCE — same caveat as this codebase's Firmex/
ShareVault/Intralinks adapters: HighQ's public API documentation is
sparse for third-party integration purposes. Endpoint paths and field
names below are a best-effort reconstruction from HighQ's general
"workspace > folder > document" model (HighQ's core Collaborate/Publisher
product organizes content into "sites"/workspaces), not a verified spec.
Trash is emulated via a hidden folder; version history is emulated as a
...
class HighQProvider(StorageProvider):
    key = "highq"
    display_name = "Thomson Reuters HighQ"
    ...
    def config_fields(self) -> list[ConfigField]:
        return [ConfigField("base_url", "HighQ site URL", "https://yourcompany.highq.com")]
    ...
    def _login(self, creds: dict) -> str:
        ...
        if resp.status_code >= 400:
            raise ProviderError("Invalid HighQ credentials", status_code=401)
    ...
    def _request(self, creds: dict, method: str, path: str, **kwargs) -> requests.Response:
        ...
        try:
            ...
            raise ProviderError(f"Couldn't reach HighQ: {exc}", status_code=502)
        ...
        if resp.status_code >= 400:
            raise ProviderError(f"HighQ error {resp.status_code}: {resp.text[:300]}", status_code=502)
        return resp
    ...
    def authenticate(self, username: str, password: str, config: dict | None = None) -> dict | None:
        ...
        if not base_url:
            raise ProviderError("HighQ site URL is required", status_code=400)
    ...
    def _workspace_id(self, creds: dict) -> str:
        workspaces = self._request(creds, "GET", "/api/v1/workspaces").json()
        entries = workspaces if isinstance(workspaces, list) else workspaces.get("data", [])
        if not entries:
            raise ProviderError("No HighQ workspace is available on this account", status_code=502)
    ...
    def _find_or_create_child(self, creds: dict, ws_id: str, parent_id: str | None, name: str) -> str:
        result = self._request(creds, "GET", f"/api/v1/workspaces/{ws_id}/folders",
                                params={"parentId": parent_id or ""}).json()
        entries = result if isinstance(result, list) else result.get("data", [])
        for e in entries:
            if e.get("name") == name:
                return str(e.get("id"))
        created = self._request(creds, "POST", f"/api/v1/workspaces/{ws_id}/folders",
    ...
    def _root_id(self, creds: dict) -> str:
        ...
        with self._root_id_lock:
            ...
            if cached:
                return cached
            ws_id = self._workspace_id(creds)
            creds["_workspace_id"] = ws_id
            root_id = self._find_or_create_child(creds, ws_id, None, _APP_ROOT_NAME)
    ...
    def _trash_id(self, creds: dict) -> str:
        ...
        with self._trash_id_lock:
            ...
            trash_id = self._find_or_create_child(creds, creds["_workspace_id"], root, _TRASH_NAME)
    ...
    def _workspace(self, creds: dict) -> str:
        self._root_id(creds)
        return creds["_workspace_id"]

    def get_children(self, creds: dict, folder_id: str | None) -> FolderContents:
        ws = self._workspace(creds)
        node = self._resolve(creds, folder_id)
        fres = self._request(creds, "GET", f"/api/v1/workspaces/{ws}/folders", params={"parentId": node}).json()
        ffiles = self._request(creds, "GET", f"/api/v1/workspaces/{ws}/documents", params={"folderId": node}).json()
    ...
    def list_trash(self, creds: dict) -> FolderContents:
        ...
        fres = self._request(creds, "GET", f"/api/v1/workspaces/{ws}/folders", params={"parentId": trash_id}).json()
        ffiles = self._request(creds, "GET", f"/api/v1/workspaces/{ws}/documents", params={"folderId": trash_id}).json()
        f_entries = fres if isinstance(fres, list) else fres.get("data", [])
        file_entries = ffiles if isinstance(ffiles, list) else ffiles.get("data", [])
    ...
    def create_folder(self, creds: dict, parent_id: str | None, name: str) -> FolderInfo:
        ...
        created = self._request(creds, "POST", f"/api/v1/workspaces/{ws}/folders",
    ...
    def rename_folder(self, creds: dict, folder_id: str, name: str) -> FolderInfo:
        ws = self._workspace(creds)
        updated = self._request(creds, "PUT", f"/api/v1/workspaces/{ws}/folders/{folder_id}",
                                 json={"name": name}).json()
    ...
    def delete_folder(self, creds: dict, folder_id: str) -> None:
        ws = self._workspace(creds)
        self._request(creds, "DELETE", f"/api/v1/workspaces/{ws}/folders/{folder_id}")
    ...
    def create_document(self, creds: dict, folder_id: str | None, name: str, content_type: str, content: bytes) -> FileInfo:
        ...
        created = self._request(creds, "POST", f"/api/v1/workspaces/{ws}/documents",
    ...
    def get_file(self, creds: dict, file_id: str) -> FileInfo:
        ws = self._workspace(creds)
        result = self._request(creds, "GET", f"/api/v1/workspaces/{ws}/documents/{file_id}").json()
        return self._entry_to_file(result.get("data", result), None)

    def rename_file(self, creds: dict, file_id: str, name: str) -> FileInfo:
        ws = self._workspace(creds)
        updated = self._request(creds, "PUT", f"/api/v1/workspaces/{ws}/documents/{file_id}",
                                 json={"name": name}).json()
    ...
    def delete_file(self, creds: dict, file_id: str) -> None:
        ws = self._workspace(creds)
        self._request(creds, "DELETE", f"/api/v1/workspaces/{ws}/documents/{file_id}")

    def get_content(self, creds: dict, file_id: str) -> bytes:
        ws = self._workspace(creds)
        return self._request(creds, "GET", f"/api/v1/workspaces/{ws}/documents/{file_id}/content").content
    ...
    def create_version(self, creds: dict, file_id: str, content_type: str, content: bytes) -> FileInfo:
        ws = self._workspace(creds)
        info = self.get_file(creds, file_id)
        self._request(creds, "POST", f"/api/v1/workspaces/{ws}/documents/{file_id}/content",
    ...
    def trash_folder(self, creds: dict, folder_id: str) -> None:
        ...
        self._request(creds, "PUT", f"/api/v1/workspaces/{ws}/folders/{folder_id}", json={"parentId": target})
    ...
    def trash_file(self, creds: dict, file_id: str) -> None:
        ws = self._workspace(creds)
        target = self._trash_id(creds)
        self._request(creds, "PUT", f"/api/v1/workspaces/{ws}/documents/{file_id}", json={"folderId": target})
    ...
    def search(self, creds: dict, query: str) -> tuple[list[FolderInfo], list[FileInfo]]:
        ...
        def walk(node_id, depth):
            if depth > 6 or node_id == trash:
                return
            fres = self._request(creds, "GET", f"/api/v1/workspaces/{ws}/folders", params={"parentId": node_id}).json()
            f_entries = fres if isinstance(fres, list) else fres.get("data", [])
            for e in f_entries:
            ...
            ffiles = self._request(creds, "GET", f"/api/v1/workspaces/{ws}/documents", params={"folderId": node_id}).json()
            file_entries = ffiles if isinstance(ffiles, list) else ffiles.get("data", [])
