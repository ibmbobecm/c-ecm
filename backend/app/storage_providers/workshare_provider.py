"""Workshare, via a REST API assumed to follow Workshare's general
documented conventions (JSON, token auth against a per-connection
Workshare site URL).

UNVERIFIED and LOW CONFIDENCE — same caveat as this codebase's Firmex/
ShareVault/Intralinks/HighQ adapters. Workshare's primary products
(document comparison, "Workshare Transact" for deal rooms) have sparse
public third-party API documentation. Endpoint paths and field names
below are a best-effort reconstruction from the general "project >
folder > document" model Workshare Transact uses, not a verified spec.
...
class WorkshareProvider(StorageProvider):
    key = "workshare"
    display_name = "Workshare"
    ...
    def config_fields(self) -> list[ConfigField]:
        return [ConfigField("base_url", "Workshare site URL", "https://yourcompany.workshare.com")]
    ...
    def _login(self, creds: dict) -> str:
        ...
        if resp.status_code >= 400:
            raise ProviderError("Invalid Workshare credentials", status_code=401)
    ...
    def _request(self, creds: dict, method: str, path: str, **kwargs) -> requests.Response:
        ...
        try:
            ...
            raise ProviderError(f"Couldn't reach Workshare: {exc}", status_code=502)
        ...
        if resp.status_code >= 400:
            raise ProviderError(f"Workshare error {resp.status_code}: {resp.text[:300]}", status_code=502)
        return resp
    ...
    def authenticate(self, username: str, password: str, config: dict | None = None) -> dict | None:
        ...
        if not base_url:
            raise ProviderError("Workshare site URL is required", status_code=400)
    ...
    def _project_id(self, creds: dict) -> str:
        projects = self._request(creds, "GET", "/api/v1/projects").json()
        entries = projects if isinstance(projects, list) else projects.get("data", [])
        if not entries:
            raise ProviderError("No Workshare project is available on this account", status_code=502)
    ...
    def _find_or_create_child(self, creds: dict, proj_id: str, parent_id: str | None, name: str) -> str:
        result = self._request(creds, "GET", f"/api/v1/projects/{proj_id}/folders",
                                params={"parentId": parent_id or ""}).json()
        entries = result if isinstance(result, list) else result.get("data", [])
        for e in entries:
            if e.get("name") == name:
                return str(e.get("id"))
        created = self._request(creds, "POST", f"/api/v1/projects/{proj_id}/folders",
    ...
    def _root_id(self, creds: dict) -> str:
        ...
        with self._root_id_lock:
            ...
            if cached:
                return cached
            proj_id = self._project_id(creds)
            creds["_project_id"] = proj_id
            root_id = self._find_or_create_child(creds, proj_id, None, _APP_ROOT_NAME)
    ...
    def _trash_id(self, creds: dict) -> str:
        ...
        with self._trash_id_lock:
            ...
            trash_id = self._find_or_create_child(creds, creds["_project_id"], root, _TRASH_NAME)
    ...
    def _project(self, creds: dict) -> str:
        self._root_id(creds)
        return creds["_project_id"]

    def get_children(self, creds: dict, folder_id: str | None) -> FolderContents:
        proj = self._project(creds)
        node = self._resolve(creds, folder_id)
        fres = self._request(creds, "GET", f"/api/v1/projects/{proj}/folders", params={"parentId": node}).json()
        ffiles = self._request(creds, "GET", f"/api/v1/projects/{proj}/documents", params={"folderId": node}).json()
    ...
    def list_trash(self, creds: dict) -> FolderContents:
        ...
        fres = self._request(creds, "GET", f"/api/v1/projects/{proj}/folders", params={"parentId": trash_id}).json()
        ffiles = self._request(creds, "GET", f"/api/v1/projects/{proj}/documents", params={"folderId": trash_id}).json()
        f_entries = fres if isinstance(fres, list) else fres.get("data", [])
        file_entries = ffiles if isinstance(ffiles, list) else ffiles.get("data", [])
    ...
    def create_folder(self, creds: dict, parent_id: str | None, name: str) -> FolderInfo:
        ...
        created = self._request(creds, "POST", f"/api/v1/projects/{proj}/folders",
    ...
    def rename_folder(self, creds: dict, folder_id: str, name: str) -> FolderInfo:
        proj = self._project(creds)
        updated = self._request(creds, "PUT", f"/api/v1/projects/{proj}/folders/{folder_id}",
                                 json={"name": name}).json()
    ...
    def delete_folder(self, creds: dict, folder_id: str) -> None:
        proj = self._project(creds)
        self._request(creds, "DELETE", f"/api/v1/projects/{proj}/folders/{folder_id}")
    ...
    def create_document(self, creds: dict, folder_id: str | None, name: str, content_type: str, content: bytes) -> FileInfo:
        ...
        created = self._request(creds, "POST", f"/api/v1/projects/{proj}/documents",
    ...
    def get_file(self, creds: dict, file_id: str) -> FileInfo:
        proj = self._project(creds)
        result = self._request(creds, "GET", f"/api/v1/projects/{proj}/documents/{file_id}").json()
        return self._entry_to_file(result.get("data", result), None)

    def rename_file(self, creds: dict, file_id: str, name: str) -> FileInfo:
        proj = self._project(creds)
        updated = self._request(creds, "PUT", f"/api/v1/projects/{proj}/documents/{file_id}",
                                 json={"name": name}).json()
    ...
    def delete_file(self, creds: dict, file_id: str) -> None:
        proj = self._project(creds)
        self._request(creds, "DELETE", f"/api/v1/projects/{proj}/documents/{file_id}")

    def get_content(self, creds: dict, file_id: str) -> bytes:
        proj = self._project(creds)
        return self._request(creds, "GET", f"/api/v1/projects/{proj}/documents/{file_id}/content").content
    ...
    def create_version(self, creds: dict, file_id: str, content_type: str, content: bytes) -> FileInfo:
        proj = self._project(creds)
        info = self.get_file(creds, file_id)
        self._request(creds, "POST", f"/api/v1/projects/{proj}/documents/{file_id}/content",
    ...
    def trash_folder(self, creds: dict, folder_id: str) -> None:
        ...
        self._request(creds, "PUT", f"/api/v1/projects/{proj}/folders/{folder_id}", json={"parentId": target})
    ...
    def trash_file(self, creds: dict, file_id: str) -> None:
        proj = self._project(creds)
        target = self._trash_id(creds)
        self._request(creds, "PUT", f"/api/v1/projects/{proj}/documents/{file_id}", json={"folderId": target})
    ...
    def search(self, creds: dict, query: str) -> tuple[list[FolderInfo], list[FileInfo]]:
        ...
        def walk(node_id, depth):
            if depth > 6 or node_id == trash:
                return
            fres = self._request(creds, "GET", f"/api/v1/projects/{proj}/folders", params={"parentId": node_id}).json()
            f_entries = fres if isinstance(fres, list) else fres.get("data", [])
            for e in f_entries:
            ...
            ffiles = self._request(creds, "GET", f"/api/v1/projects/{proj}/documents", params={"folderId": node_id}).json()
            file_entries = ffiles if isinstance(ffiles, list) else ffiles.get("data", [])
