"""Intralinks (virtual data rooms / VIA platform), via a REST API assumed
to follow Intralinks' general documented conventions (JSON, token auth
against a per-connection Intralinks site URL).

UNVERIFIED and LOW CONFIDENCE — same caveat as this codebase's Firmex and
ShareVault adapters: Intralinks' public API documentation is sparse
(enterprise VDR access is typically customer-specific). Endpoint paths
and field names below are a best-effort reconstruction from the general
"exchange > folder > document" model common to this product category
(Intralinks calls a data room an "Exchange"), not a verified spec. Trash
is emulated via a hidden folder; version history is emulated as a single
current version.
"""
...
class IntralinksProvider(StorageProvider):
    key = "intralinks"
    display_name = "Intralinks"
    ...
    def config_fields(self) -> list[ConfigField]:
        return [ConfigField("base_url", "Intralinks site URL", "https://api.intralinks.com")]

    def _base_url(self, creds: dict) -> str:
        return creds["base_url"].rstrip("/")

    def _login(self, creds: dict) -> str:
        resp = requests.post(f"{self._base_url(creds)}/auth/v1/login",
                              json={"username": creds["username"], "password": creds["password"]}, timeout=30)
        if resp.status_code >= 400:
            raise ProviderError("Invalid Intralinks credentials", status_code=401)
    ...
    def _request(self, creds: dict, method: str, path: str, **kwargs) -> requests.Response:
        ...
        try:
            ...
            raise ProviderError(f"Couldn't reach Intralinks: {exc}", status_code=502)
        ...
        if resp.status_code >= 400:
            raise ProviderError(f"Intralinks error {resp.status_code}: {resp.text[:300]}", status_code=502)
        return resp
    ...
    def authenticate(self, username: str, password: str, config: dict | None = None) -> dict | None:
        ...
        if not base_url:
            raise ProviderError("Intralinks site URL is required", status_code=400)
    ...
    def _exchange_id(self, creds: dict) -> str:
        exchanges = self._request(creds, "GET", "/exchanges/v1/exchanges").json()
        entries = exchanges if isinstance(exchanges, list) else exchanges.get("data", [])
        if not entries:
            raise ProviderError("No Intralinks exchange is available on this account", status_code=502)
    ...
    def _find_or_create_child(self, creds: dict, exch_id: str, parent_id: str | None, name: str) -> str:
        result = self._request(creds, "GET", f"/exchanges/v1/exchanges/{exch_id}/folders",
                                params={"parentId": parent_id or ""}).json()
        entries = result if isinstance(result, list) else result.get("data", [])
        for e in entries:
            if e.get("name") == name:
                return str(e.get("id"))
        created = self._request(creds, "POST", f"/exchanges/v1/exchanges/{exch_id}/folders",
    ...
    def _root_id(self, creds: dict) -> str:
        ...
        with self._root_id_lock:
            ...
            if cached:
                return cached
            exch_id = self._exchange_id(creds)
            creds["_exchange_id"] = exch_id
            root_id = self._find_or_create_child(creds, exch_id, None, _APP_ROOT_NAME)
    ...
    def _trash_id(self, creds: dict) -> str:
        ...
        with self._trash_id_lock:
            ...
            trash_id = self._find_or_create_child(creds, creds["_exchange_id"], root, _TRASH_NAME)
    ...
    def _exchange(self, creds: dict) -> str:
        self._root_id(creds)
        return creds["_exchange_id"]

    def get_children(self, creds: dict, folder_id: str | None) -> FolderContents:
        exch = self._exchange(creds)
        node = self._resolve(creds, folder_id)
        fres = self._request(creds, "GET", f"/exchanges/v1/exchanges/{exch}/folders", params={"parentId": node}).json()
        ffiles = self._request(creds, "GET", f"/exchanges/v1/exchanges/{exch}/documents", params={"folderId": node}).json()
    ...
    def list_trash(self, creds: dict) -> FolderContents:
        ...
        fres = self._request(creds, "GET", f"/exchanges/v1/exchanges/{exch}/folders", params={"parentId": trash_id}).json()
        ffiles = self._request(creds, "GET", f"/exchanges/v1/exchanges/{exch}/documents", params={"folderId": trash_id}).json()
        f_entries = fres if isinstance(fres, list) else fres.get("data", [])
        file_entries = ffiles if isinstance(ffiles, list) else ffiles.get("data", [])
    ...
    def create_folder(self, creds: dict, parent_id: str | None, name: str) -> FolderInfo:
        ...
        created = self._request(creds, "POST", f"/exchanges/v1/exchanges/{exch}/folders",
    ...
    def rename_folder(self, creds: dict, folder_id: str, name: str) -> FolderInfo:
        exch = self._exchange(creds)
        updated = self._request(creds, "PUT", f"/exchanges/v1/exchanges/{exch}/folders/{folder_id}",
                                 json={"name": name}).json()
    ...
    def delete_folder(self, creds: dict, folder_id: str) -> None:
        exch = self._exchange(creds)
        self._request(creds, "DELETE", f"/exchanges/v1/exchanges/{exch}/folders/{folder_id}")
    ...
    def create_document(self, creds: dict, folder_id: str | None, name: str, content_type: str, content: bytes) -> FileInfo:
        ...
        created = self._request(creds, "POST", f"/exchanges/v1/exchanges/{exch}/documents",
    ...
    def get_file(self, creds: dict, file_id: str) -> FileInfo:
        exch = self._exchange(creds)
        result = self._request(creds, "GET", f"/exchanges/v1/exchanges/{exch}/documents/{file_id}").json()
        return self._entry_to_file(result.get("data", result), None)

    def rename_file(self, creds: dict, file_id: str, name: str) -> FileInfo:
        exch = self._exchange(creds)
        updated = self._request(creds, "PUT", f"/exchanges/v1/exchanges/{exch}/documents/{file_id}",
                                 json={"name": name}).json()
    ...
    def delete_file(self, creds: dict, file_id: str) -> None:
        exch = self._exchange(creds)
        self._request(creds, "DELETE", f"/exchanges/v1/exchanges/{exch}/documents/{file_id}")

    def get_content(self, creds: dict, file_id: str) -> bytes:
        exch = self._exchange(creds)
        return self._request(creds, "GET", f"/exchanges/v1/exchanges/{exch}/documents/{file_id}/content").content
    ...
    def create_version(self, creds: dict, file_id: str, content_type: str, content: bytes) -> FileInfo:
        exch = self._exchange(creds)
        info = self.get_file(creds, file_id)
        self._request(creds, "POST", f"/exchanges/v1/exchanges/{exch}/documents/{file_id}/content",
    ...
    def trash_folder(self, creds: dict, folder_id: str) -> None:
        ...
        self._request(creds, "PUT", f"/exchanges/v1/exchanges/{exch}/folders/{folder_id}", json={"parentId": target})
    ...
    def trash_file(self, creds: dict, file_id: str) -> None:
        exch = self._exchange(creds)
        target = self._trash_id(creds)
        self._request(creds, "PUT", f"/exchanges/v1/exchanges/{exch}/documents/{file_id}", json={"folderId": target})
    ...
    def search(self, creds: dict, query: str) -> tuple[list[FolderInfo], list[FileInfo]]:
        ...
        def walk(node_id, depth):
            if depth > 6 or node_id == trash:
                return
            fres = self._request(creds, "GET", f"/exchanges/v1/exchanges/{exch}/folders", params={"parentId": node_id}).json()
            f_entries = fres if isinstance(fres, list) else fres.get("data", [])
            for e in f_entries:
            ...
            ffiles = self._request(creds, "GET", f"/exchanges/v1/exchanges/{exch}/documents", params={"folderId": node_id}).json()
            file_entries = ffiles if isinstance(ffiles, list) else ffiles.get("data", [])
